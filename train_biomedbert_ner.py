...
def tokenize_and_align_labels(examples):
    ...
    # Karena satu teks bisa jadi banyak 'window', kita perlu map window ke index teks aslinya
...
# Gunakan remove_columns karena jumlah baris setelah tokenisasi (windows) 
# akan lebih banyak daripada jumlah teks asli
tokenized_ds = ds.map(
...
training_args = TrainingArguments(
    output_dir="./biomedbert_sliding_window70",
    eval_strategy="epoch",
    save_strategy="epoch",
    learning_rate=2e-5,            # Dinaikkan dari 1e-5
    per_device_train_batch_size=8, # Jika RAM GPU kuat, naikkan ke 8 atau 16
    num_train_epochs=50,           # Memberi waktu lebih banyak untuk belajar pola
    weight_decay=0.01,             # Regularisasi agar tidak menghafal (overfit)
    warmup_ratio=0.1,              # 10% awal training untuk pemanasan LR
    lr_scheduler_type="linear",
    load_best_model_at_end=True,   
    metric_for_best_model="eval_loss",
    greater_is_better=False,           # Karena loss, maka semakin kecil semakin baik 
...
if __name__ == "__main__":
    ...
    trainer.train()
    trainer.save_model("./biomedbert_sliding_window70")
    print("Selesai!")